...
export default function MapView({
  ...
  function handlePickFromModal(item: WifiNetwork) {
    ...
    if (marker && cluster) {
      cluster.zoomToShowLayer(marker as any, () => {
  ...
  return (
    <div className={className} style={{ width: '100%', position: 'relative' }}>
      ...
      >
        ...
        <MarkerClusterGroup
          // ⚠️ impede o comportamento padrão de clicar no cluster
          zoomToBoundsOnClick={false}
          spiderfyOnMaxZoom={false}
          // alguns builds expõem esta prop; manter false não machuca
          // @ts-ignore
          ...
          whenCreated={(cg: L.MarkerClusterGroup) => {
            clusterRef.current = cg;

            // Abre modal com as redes do cluster
            cg.on('clusterclick', (e: any) => {
              // mata o clique padrão (zoom/spiderfy) do plugin
