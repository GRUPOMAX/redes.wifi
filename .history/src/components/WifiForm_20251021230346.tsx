...
export default function WifiForm({ initial = null, onSave, onCancelEdit }: WifiFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Sucesso: limpa tudo SEM dó
      clearAll();

      // Se estava editando, sai do modo edição
      if (initial && onCancelEdit) onCancelEdit();
    } catch (e: any) {
    ...
    <>
      <form onSubmit={handleSubmit} className="space-y-4" id="wifi-form">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute inset-y-0 right-2 my-auto inline-flex h-8 w-8 items-center justify-center
                           rounded-md border border-white/10 bg-white/5 hover:bg-white/10 transition"
                disabled={submitting}
              >
                <MapPin size={18} />
              </button>
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={getMyLocation} className="btn" disabled={submitting}>
            Usar minha localização
          </button>
          <div className="flex-1" />
          {editing && onCancelEdit && (
            <button type="button" onClick={onCancelEdit} className="btn" disabled={submitting}>
              Cancelar
            </button>
          )}
