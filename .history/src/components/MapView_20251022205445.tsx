...
export default function MapView({
  ...
  function handlePickFromModal(item: WifiNetwork) {
    ...
    // fecha modal para liberar interação
    setModalOpen(false);

    if (!marker || !map) return;

    // dá um tick pro mapa recalcular layout
    setTimeout(() => map.invalidateSize(), 0);

    const latlng = marker.getLatLng();

    const focusOnMarker = () => {
      // força o foco e o zoom (independente do zoom atual)
      const targetZoom = 19; // pode subir pra 20/21 se quiser aproximar mais
      setFocus(id);
      map.flyTo(latlng, targetZoom, { animate: true, duration: 0.6 });
      map.once('moveend', () => (marker as any).openPopup?.());
    };

    // tenta abrir o caminho se estiver em cluster, mas com fallback garantido
    let finished = false;
    const done = () => {
      if (finished) return;
      finished = true;
      // 1 micro-tick pra garantir que o marker está “no mapa” após o zts
      setTimeout(focusOnMarker, 10);
    };

    const zts = (clusterGroup as any)?.zoomToShowLayer;
    if (typeof zts === 'function' && (clusterGroup as any)?.hasLayer?.(marker)) {
      zts(marker as any, done);
      // fallback duro caso o plugin não chame o callback
      setTimeout(done, 700);
    } else {
      done();
    }
...
>
  <TileLayer
    ...
    maxZoom={21}             // ⬅️ opcional mas recomendado
  />
