...
// Corrige ícones padrão do Leaflet (se você não estiver usando divIcon custom)
...
export default function MapView({
  ...
  // use nome diferente para evitar colisão com possíveis props "points"
    ...
    <div className={className} style={{ width: '100%', position: 'relative' }}>
      ...
      >
              ...
              <Marker
                ...
                ref={(ref) => {
                  if (ref) {
                    ...
                    markerByIdRef.current.set(getWifiId(raw), ref as unknown as L.Marker);
                  } else {
              ...
              >
                <Popup className="wifi-popup" maxWidth={320} closeButton>
                  <div className="space-y-1">
                    <div className="text-sm font-semibold">{(raw as any)['NOME-WIFI'] ?? (raw as any).NOME ?? 'Wi-Fi'}</div>
                    {'ENDERECO' in (raw as any) && (
                      <div className="text-xs opacity-70">{(raw as any).ENDERECO}</div>
                    )}
                    {'SENHA-WIFI-2G' in (raw as any) && (
                      <div className="text-xs">Senha 2G: <strong>{(raw as any)['SENHA-WIFI-2G'] || '-'}</strong></div>
                    )}
                    {'SENHA-WIFI-5G' in (raw as any) && (
                      <div className="text-xs">Senha 5G: <strong>{(raw as any)['SENHA-WIFI-5G'] || '-'}</strong></div>
                    )}
                  </div>
                </Popup>
              </Marker>
        ...
        <FitAllOnce
          ...
          points={(wifiPoints ?? []).map((p) => ({ lat: p.lat, lng: p.lng }))}
        />
      </MapContainer>

      {/* estilos mínimos inline (pode mover pro seu CSS) */}
      <style>{`
