...
export default function MapModal({ open, title, center, onClose }: Props) {
  ...
  // Fecha com ESC (hook sempre declarado; lógica condicional dentro)
  useEffect(() => {
    if (!open) return;
  ...
  // Recalibra o mapa quando abre / quando center muda
  useEffect(() => {
  ...
  function recenter() {
    ...
    mapRef.current?.setView(
      [lat, lng],
      Math.max(18, mapRef.current?.getZoom() ?? 18),
      { animate: true }
    );
  }
  async function copyCoords() {
    ...
    try {
      ...
      // Evita no-empty; oferece fallback básico
      console.warn("Falha ao copiar coords:", err);
      alert(`${lat.toFixed(6)}, ${lng.toFixed(6)}`);
    }
  }

  // Render condicional pode ocorrer aqui, DEPOIS que todos os hooks foram chamados
