...
export default function MapModal({ open, title, center, onClose }: Props) {
  ...
  // Ícone Wi-Fi consistente (não depende do default do Leaflet)
  ...
  // Trava scroll da página quando o modal está aberto
  useEffect(() => {
    if (!open) return;
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 bg-black/30">
          ...
          <div className="ml-auto flex items-center gap-2">
            <button onClick={recenter} className="px-3 py-1.5 text-sm rounded-lg bg-white/10 hover:bg-white/15 border border-white/10">
              Recentrar
            </button>
            ...
            <button onClick={copyCoords} className="px-3 py-1.5 text-sm rounded-lg bg-white/10 hover:bg-white/15 border border-white/10">
              Copiar coords
            </button>
            <button
              onClick={onClose}
              className="px-3 py-1.5 text-sm rounded-lg bg-emerald-400 text-[#0b1220] font-semibold hover:brightness-105"
              title="Esc"
            >
              Fechar
            </button>
        ...
        <MapContainer
          ...
          touchZoom={true}
          style={{ width: "100%", height: "calc(100% - 56px)" }} // desconta header
        >
          <TileLayer
